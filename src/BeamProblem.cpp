...
void BeamProblem::write_solutions_to_file(std::ofstream &outfile){
    outfile << "x,jacobi1,jacobi2,jacobi3,analytic1,analytic2,analytic3" << std::endl;
    outfile << "0,0,0,0,0,0,0"<<std::endl;
    for (int i = 0; i < N; i++){
        outfile << (i + 1) * h 
        << "," << jacobi_eigenvecs(i, 0) << "," << jacobi_eigenvecs(i, 1) << "," << jacobi_eigenvecs(i, 2) << "," 
        << analytical_eigenvecs(i, 0) << "," << analytical_eigenvecs(i, 1) << "," << analytical_eigenvecs(i, 2) 
        << std::endl;
    }
    outfile << "1,0,0,0,0,0,0" << std::endl;
}