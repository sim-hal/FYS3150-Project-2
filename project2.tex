...
\begin{equation}\label{eq:dimensionlessdifferentialequation}
    \frac{d^2 u(\hat{x})}{d x^2} = -\lambda u(\hat{x})
\end{equation}

where $\hat{x}\equiv x/L \iff x$ is a dimensionless  variable and $\lambda=\frac{F L^2}{\gamma}$. This also means that $x = \hat{x} L$. This means that

\begin{equation*}
    \frac{d}{dx} = \frac{d \hat{x}}{x}\frac{d}{d \hat{x}} = \frac{1}{L}\frac{d x}{x}\frac{d}{d \hat{x}} =\frac{1}{L} \frac{d}{d\hat{x}}
\end{equation*}

We insert this into \eqref{eq:differentialequation} and get

\begin{equation*}
    \gamma \frac{d^2 u(x)}{d x^2} = \gamma \frac{1}{L^2} \frac{d^2 u(\hat{x})}{d \hat{x}^2} = -F u(x) 
\end{equation*}

% TODO: HOW TO REMOVE x FROM LAST EXPRESSION?
\begin{equation*}
    \frac{d^2 u(\hat{x})}{d \hat{x}^2} = -F L^2 \frac{1}{\gamma } u(x)  = - \lambda u(x)
\end{equation*}
